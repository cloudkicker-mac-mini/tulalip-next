(function () {
  ...
  function icon(name, cls="") { const node=el("i",`ki-outline ki-${name} ${cls}`.trim()); node.setAttribute("aria-hidden","true"); return node; }
  ...
  function iconFor(c){ return ({Health:"heart-circle",Government:"bank",Elders:"people",Family:"people",Culture:"abstract-26",Community:"people",Business:"briefcase"})[c]||"calendar-8"; }
  function categoryLabel(category, light=false){const label=el("span",light?"category light":"category");label.append(icon(iconFor(category)),document.createTextNode(category));return label;}

  function reminderButton(event, label="Add to calendar") { const b=el("button","btn calendar-button"); b.type="button"; b.append(icon("calendar-add"),document.createTextNode(label)); b.addEventListener("click",e=>{e.stopPropagation(); downloadICS(event);}); return b; }
  function eventCard(event) {
    ...
    const body=el("div","event-card-body"), meta=el("div","card-meta"); meta.append(categoryLabel(event.category),el("span","type",event.type));
    body.append(meta,el("h3","",event.title),el("p","event-time",`${event.timeLabel} · ${event.location}`));
    const arrow=el("span","card-arrow"); arrow.append(icon("right")); card.append(date,body,arrow);
  ...
  function renderNext(){ const now=new Date(), event=events.find(item=>parseEventDate(item.end)>=now); if(!event){const wrap=el("article","next-card card");wrap.append(el("h2","","You’re all caught up"),el("p","event-time","No future dates remain in this newsletter issue."));next.replaceChildren(wrap);return;} const wrap=el("article","next-card card"); const top=el("div","next-top"); top.append(el("span","next-badge",relative(event)),categoryLabel(event.category,true)); wrap.append(top,el("h2","",event.title)); const details=el("div","next-details"),time=el("div"),place=el("div");time.append(icon("time"),document.createTextNode(event.timeLabel));place.append(icon("geolocation"),document.createTextNode(event.location));details.append(time,place); wrap.append(details); const actions=el("div","next-actions"), view=el("button","btn secondary-button","View details"); view.addEventListener("click",()=>openEvent(event)); actions.append(reminderButton(event,"Remind me"),view); wrap.append(actions,el("p","calendar-note","Downloads an event to your device calendar, where you can choose the reminder time.")); next.replaceChildren(wrap); }
  ...
  function openEvent(event){
    const head=el("div","dialog-head"); head.append(categoryLabel(event.category),el("span","type",event.type));
